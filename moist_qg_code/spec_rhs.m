...
path = [pwd,'/snapshots/snapshots_s2.h5'];
...
semilogx(kx(1:nx/2),power_rhs_kx(1:nx/2)); hold on; %/max(power_rhs_kx(1:nx/2)))
ylabel('Magnitude')
xlabel('kx')
xlim([kx(1) kx(nx/2)])
title('\rm 1D-Spectrum x')
...
subplot(1,2,1)
loglog(kx(1:nx/2),power_rhs_kx(1:nx/2)) %/max(power_rhs_kx(1:nx/2)))
...
subplot(1,2,2)
loglog(ky(1:ny/2),power_rhs_ky(1:ny/2)) %/max(power_rhs_ky(1:ny/2)))
ylabel('abs(fft(rhs))^2')
xlabel('ky')
xlim([ky(1) ky(ny/2)])
title('1D-Spectrum y')
