...
def readslice(inputfilename,nx,ny,nfile):
    ...
    field = np.fromfile(f,dtype = 'd',count=int(nx*ny/nfile))         #gets the data in a 1x(nx*ny/nfile) vector, later to be transformed into 2D array, once stitched together
    f.close()
    return field
